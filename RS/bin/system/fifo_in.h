...
--------------------------------------------------------------------------------
*/
#ifndef __FIFO_IN_H__
#define __FIFO_IN_H__
...
////////////////////////////////////////////////////////////////////////////////
{
    ...
    // Signals
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > state_reg;    // current FIFO state
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > next_state;   // next    FIFO state
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > rd_ptr_reg;   // read  pointer
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > wr_ptr_reg;   // write pointer
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > next_rd_ptr;  // next	read 	pointer
    sc_signal<sc_uint<FIFO_IN_LOG2_DEPTH+1> > next_wr_ptr;  // next write pointer
    sc_signal<sc_uint<FIFO_WIDTH> > fifo[FIFO_IN_DEPTH];    // here is the buffer
    ...
    SC_HAS_PROCESS(fifo_in);

    //////////////////////////////////////////////////////////////////////////////
    fifo_in(sc_module_name nm,	unsigned short int XID, unsigned short int YID,
...
#ifdef DEBUG_FIFO_IN_CONTROLLER
...
#ifdef DEBUG_FIFO_IN_DATAPATH
        SC_METHOD(p_debug_datapath);
        sensitive << clk.pos();
#endif

        //////////////////////////
#else // FIFO_IN_DEPTH = 0
        //////////////////////////
